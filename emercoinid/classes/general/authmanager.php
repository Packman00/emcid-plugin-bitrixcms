...
use Bitrix\Emercoinid\ContactTable;
...
class CEmcAuthManager
{
	...
	public function __construct($userId = null)
	{
		...
		if(!is_array(self::$arAuthServices))
		{
			...
			foreach(GetModuleEvents("emercoinid", "OnAuthServicesBuildList", true) as $arEvent)
			{
	...
	protected static function AppyUserSettings($suffix)
	{
		...
		$arServices = unserialize(COption::GetOptionString("emercoinid", "auth_services".$suffix, ""));
	...
	public function GetProfileUrl($service, $uid, $arService = false)
	{
		if(isset(self::$arAuthServices[$service]))
		{
			...
			{
				$dbSocservUser = \CEmcAuthDB::getList(
					array(),
					array(
						'USER_ID' => $this->userId,
	...
	public static function GetUserArrayForSendMessages($userId)
	{
		...
		if($userId > 0)
		{
			$dbSocservUser = CEmcAuthDB::GetList(array(), array('USER_ID' => $userId), false, false, array("ID", "EXTERNAL_AUTH_ID", "OATOKEN"));
	...
	public static function SendUserMessage($socServUserId, $providerName, $message, $messageId)
	{
		...
		if($providerName != '' && $socServUserId > 0)
		{
			...
			{
				case 'Twitter':
					$className = "CEmcTwitter";
					break;
				case 'Facebook':
					$className = "CEmcFacebook";
					break;
				case 'Odnoklassniki':
					$className = "CEmcOdnoklassniki";
					break;
				default:
					$className = "";
			}
		...
		if(isset($arUserTwit['statuses']) && !empty($arUserTwit['statuses']))
		{
			foreach($arUserTwit['statuses'] as $userTwit)
			{
				...
				if(IsModuleInstalled('bitrix24') && defined('BX24_HOST_NAME'))
				{
					...
					$arUser = $rsUser->Fetch();
					foreach(GetModuleEvents("emercoinid", "OnPublishEmcMessage", true) as $arEvent)
	...
	function GetTwitMessages($lastTwitId = "1", $counter = 1)
	{
		...
		$arActiveEmc = $oAuthManager->GetActiveAuthServices(array());
		if(!(isset($arActiveEmc["Twitter"]) && isset($arActiveEmc["Twitter"]["__active"])) || !function_exists("hash_hmac"))
			return false;
		if(!CModule::IncludeModule("socialnetwork"))
			return "CEmcAuthManager::GetTwitMessages(\"$lastTwitId\", $counter);";
		global $USER;
		...
		if(intval($lastTwitId) <= 1 || $counter == 1)
			$lastTwitId = COption::GetOptionString('emercoinid', 'last_twit_id', '1');
		...
		if($counter >= 20)
		{
			// $oldLastId = COption::GetOptionString('emercoinid', 'last_twit_id', '1');
			// if((strlen($lastTwitId) > strlen($oldLastId)) && $oldLastId[0] != 9)
			// 	$lastTwitId = substr($lastTwitId, 1);
			COption::SetOptionString('emercoinid', 'last_twit_id', $lastTwitId);
			$counter = 1;
		}
	...
	public static function SendSocialservicesMessages()
	{
		...
		if($obCache->InitCache($ttl, $cache_id, $cache_dir))
			$arEmcMessage = $obCache->GetVars();
		else
		{
			$dbEmcMessage = CEmcMessage::GetList(array(), array('SUCCES_SENT' => 'N'), false, array("nTopCount" => 5), array("ID", "SOCSERV_USER_ID", "PROVIDER", "MESSAGE"));

			while($arSocMessage = $dbEmcMessage->Fetch())
				$arEmcMessage[] = $arSocMessage;
			if(empty($arEmcMessage))
				if($obCache->StartDataCache())
					$obCache->EndDataCache($arEmcMessage);
		}
		if(is_array($arEmcMessage) && !empty($arEmcMessage))
			foreach($arEmcMessage as $arSocMessage)
			{
				$arResult = CEmcAuthManager::SendUserMessage($arSocMessage['SOCSERV_USER_ID'], $arSocMessage['PROVIDER'], $arSocMessage['MESSAGE'], $arSocMessage['ID']);
				if($arResult !== false && is_array($arResult) && !preg_match("/error/i", join(",", array_keys($arResult))))
					self::MarkMessageAsSent($arSocMessage['ID']);
			}
		return "CEmcAuthManager::SendSocialservicesMessages();";
	}

	private static function MarkMessageAsSent($id)
	{
		CEmcMessage::Update($id, array("SUCCES_SENT" => 'Y'));
	}
	...
	public function GetUserArray($authId)
	{
		...
		else
		{
			...
			$dbSocUser = CEmcAuthDB::GetList(array(), array('EXTERNAL_AUTH_ID' => $authId, "ACTIVE" => 'Y'), false, false, array("XML_ID", "USER_ID", "OATOKEN", "OASECRET", "SITE_ID"));
	...
	public static function GetCachedUserOption($option)
	{
		...
		if(is_object($USER))
		{
			...
			{
				$result = CUtil::JSEscape(CUserOptions::GetOption("emercoinid", $option, "N", $USER->GetID()));
				if($obCache->StartDataCache())
...
class CEmcAuth
{
	...
	protected function CheckFields($action, &$arFields)
	{
		...
		if($action === 'ADD')
		{
			...
			$dbCheck = CEmcAuthDB::GetList(array(), array("USER_ID" => $arFields["USER_ID"], "EXTERNAL_AUTH_ID" => $arFields["EXTERNAL_AUTH_ID"]), false, false, array("ID"));
			if($dbCheck->Fetch())
			{
				return false;
			}
		...
		if(is_set($arFields, "PERSONAL_PHOTO"))
		{
			...
			{
				$arFields["PERSONAL_PHOTO"]["MODULE_ID"] = "emercoinid";
				CFile::SaveForDB($arFields, "PERSONAL_PHOTO", "emercoinid");
			}
		}
	...
	static function Update($id, $arFields)
	{
		...
		foreach(GetModuleEvents("emercoinid", "OnBeforeEmcUserUpdate", true) as $arEvent)
		{
		...
		$strUpdate = $DB->PrepareUpdate("b_emercoinid_user", $arFields);

		$strSql = "UPDATE b_emercoinid_user SET ".$strUpdate." WHERE ID = ".$id." ";
		...
		$arFields['ID'] = $id;
		foreach(GetModuleEvents("emercoinid", "OnAfterEmcUserUpdate", true) as $arEvent)
	...
	public static  function Delete($id)
	{
		...
		if ($id > 0)
		{
			$rsUser = $DB->Query("SELECT ID FROM b_emercoinid_user WHERE ID=".$id);
			$arUser = $rsUser->Fetch();
			if(!$arUser)
				return false;

			foreach(GetModuleEvents("emercoinid", "OnBeforeEmcUserDelete", true) as $arEvent)
				ExecuteModuleEventEx($arEvent, array($id));

			$DB->Query("DELETE FROM b_emercoinid_user WHERE ID = ".$id." ", true);
			$cache_id = 'socserv_ar_user';
			$obCache = new CPHPCache;
			$cache_dir = '/bx/socserv_ar_user';
			$obCache->Clean($cache_id, $cache_dir);
	...
	function OnUserDelete($id)
	{
		...
		if ($id > 0)
		{
			$DB->Query("DELETE FROM b_emercoinid_user WHERE USER_ID = ".$id." ", true);
			return true;
		}
		return false;
	}
	...
	{
		if(COption::GetOptionString("emercoinid", "allow_send_user_activity", "Y") != 'Y')
			return;
		global $USER;
		...
		{
			$enabledSendMessage = CUserOptions::GetOption("emercoinid", "user_socserv_enable", "N", $arResult['USER_ID']);
			if($enabledSendMessage == 'Y')
			{
				$enabledEndDaySend = CUserOptions::GetOption("emercoinid", "user_socserv_end_day", "N", $arResult['USER_ID']);
				if($enabledEndDaySend == 'Y')
				{
					$arResult['MESSAGE'] = str_replace('#event#', $eventCounter, str_replace('#task#', $taskCounter, CUserOptions::GetOption("emercoinid", "user_socserv_end_text", GetMessage("JS_CORE_SS_WORKDAY_START"), $arResult['USER_ID'])));

					$socServArray = CUserOptions::GetOption("emercoinid", "user_socserv_array", "a:0:{}", $arResult['USER_ID']);
					if(!CheckSerializedData($socServArray))
					{
						$socServArray = "a:0:{}";
					}

					$arEmcUser['SOCSERVARRAY'] = unserialize($socServArray);

					if(is_array($arEmcUser['SOCSERVARRAY']) && count($arEmcUser['SOCSERVARRAY']) > 0)
					{
						foreach($arEmcUser['SOCSERVARRAY'] as $id => $providerName)
						{
							$arResult['SOCSERV_USER_ID'] = $id;
							$arResult['PROVIDER'] = $providerName;
							CEmcMessage::Add($arResult);
						}
					}
				}
			}
	...
	function OnAfterTMDayStart()
	{
		...
		if($arResult['USER_ID'] > 0)
		{
			...
			if($enabledSendMessage == 'Y')
			{
				$enabledEndDaySend = CUserOptions::GetOption("emercoinid", "user_socserv_start_day", "N", $arResult['USER_ID']);
				if($enabledEndDaySend == 'Y')
				{
					$arResult['MESSAGE'] = CUserOptions::GetOption("emercoinid", "user_socserv_start_text", GetMessage("JS_CORE_SS_WORKDAY_START"), $arResult['USER_ID']);
	...
	public static function OptionsSuffix()
	{
		...
		$arUseOnSites = unserialize(COption::GetOptionString("emercoinid", "use_on_sites", ""));
	...
	public static function GetOption($opt)
	{
		...
		return COption::GetOptionString("emercoinid", $opt.self::$settingsSuffix);
	}
	...
	public static function SetOption($opt, $value)
	{
		...
		return COption::SetOptionString("emercoinid", $opt.self::$settingsSuffix, $value);
	}

	public static function getGroupsDenyAuth()
	{
		return explode(',', (\COption::GetOptionString("emercoinid", "group_deny_auth", "")));
	}

	public static function getGroupsDenySplit()
	{
		return explode(',', (\COption::GetOptionString("emercoinid", "group_deny_split", "")));
	}

	public static function setGroupsDenyAuth($value)
	{
		\COption::SetOptionString('emercoinid', 'group_deny_auth', is_array($value) ? implode(',', $value) : '');
	}

	public static function setGroupsDenySplit($value)
	{
		\COption::SetOptionString('emercoinid', 'group_deny_split', is_array($value) ? implode(',', $value) : '');
	}
	...
	public function AuthorizeUser($socservUserFields)
	{
		...
		else
		{
			...
			{
				// check for user with old emercoinid linking system (socservice ID in user's EXTERNAL_AUTH_ID)
				...
				{
					// theoretically possible situation with abandoned external user w/o b_emercoinid_user entry
					...
					(
						...
						&& COption::GetOptionString("emercoinid", "allow_registration", "Y") == "Y"
					)
					{
			...
			if($USER_ID > 0)
			{
				...
				if($USER->IsJustAuthorized())
				{
					...
					foreach(GetModuleEvents("emercoinid", "OnUserLoginSocserv", true) as $arEvent)
					{
...
class CEmcAllMessage
{
	...
	static function Update($id, $arFields)
	{
		...
		if($id<=0 || !self::CheckFields('UPDATE', $arFields))
			return false;
		$strUpdate = $DB->PrepareUpdate("b_emercoinid_message", $arFields);
		$strSql = "UPDATE b_emercoinid_message SET ".$strUpdate." WHERE ID = ".$id." ";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
		$cache_id = 'socserv_mes_user';
		$obCache = new CPHPCache;
		$cache_dir = '/bx/socserv_mes_user';
	...
	static function Delete($id)
	{
		...
		if ($id > 0)
		{
			$rsUser = $DB->Query("SELECT ID FROM b_emercoinid_message WHERE ID=".$id);
			$arUser = $rsUser->Fetch();
			if(!$arUser)
				return false;

			$DB->Query("DELETE FROM b_emercoinid_message WHERE ID = ".$id." ", true);
			$cache_id = 'socserv_mes_user';
			$obCache = new CPHPCache;
			$cache_dir = '/bx/socserv_mes_user';
			$obCache->Clean($cache_id, $cache_dir);
