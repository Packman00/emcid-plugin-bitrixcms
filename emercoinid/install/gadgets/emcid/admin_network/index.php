...
if(Loader::includeModule('emercoinid'))
{
	if(Option::get("emercoinid", "bitrix24net_id", "") == "")
	{
		$url = "/bitrix/admin/settings.php?lang=".LANGUAGE_ID."&mid=emercoinid&register_network=yes";
	}
	else
	{
		$dbRes = \Bitrix\Emercoinid\UserTable::getList(array(
			'filter' => array(
				'=USER_ID' => $USER->GetID(),
		...
		{
			$url = "/bitrix/admin/user_edit.php?lang=".LANGUAGE_ID."&ID=".$USER->GetID()."&user_edit_active_tab=edit_emercoinid";
...
{
	$url = "/bitrix/admin/module_admin.php?&action=&lang=".LANGUAGE_ID."&id=emercoinid&install=1&".bitrix_sessid_get();
}
?>
