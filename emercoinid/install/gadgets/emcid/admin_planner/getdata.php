...
CModule::IncludeModule('emercoinid');
...
$savedPortalURI = CUserOptions::GetOption('emercoinid', 'bitrix24_task_planer_gadget_portal', '');
$requestCode = CUserOptions::GetOption('emercoinid', 'bitrix24_task_planer_gadget_code', '');

if($savedPortalURI !== $portalURI || $savedPortalURI == '')
{
	$needAuthorize = true;
	CUserOptions::SetOption('emercoinid', 'bitrix24_task_planer_gadget_portal', $portalURI, false);
}
if(!preg_match('|^http[s]?|', $portalURI))
	$portalURI = 'https://'.$portalURI;
$objBitrixOAuth = new CEmcBitrixOAuth($clientId, $clientSecret, $portalURI, $redirectURI, $USER->GetID());
...
if($requestCode <> '')
{
	...
	CUserOptions::SetOption('emercoinid', 'bitrix24_task_planer_gadget_code', '', false);
}
else
{
