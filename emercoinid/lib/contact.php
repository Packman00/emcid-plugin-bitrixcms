...
class ContactTable extends Main\Entity\DataManager
{
	...
	protected static function processPossibleContacts($owner, array $contactsList)
	{
		...
		$dbRes = UserLinkTable::getList(array(
			'filter' => array(
				...
				'=SOCSERV_USER.EXTERNAL_AUTH_ID' => \CEmcEmercoinIDAuth::ID,
			),
	...
	protected static function notifyPossible($userId)
	{
		...
		{
			$ts = time();
			$alreadyShown = \CUserOptions::GetOption("emercoinid", "possible_contacts", null, $userId);
...
*/
			...
			\CUserOptions::SetOption("emercoinid", "possible_contacts", $alreadyShown, false, $userId);
		}
	}
}