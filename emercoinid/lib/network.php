...
class Network
{
	...
	{
		return Option::get('emercoinid', 'network_enable', 'N') == 'Y';
	}
	...
	public function setEnable($enable = true)
	{
		...
		Option::set('emercoinid', 'network_enable', $enable? 'Y': 'N');
	...
	public function addUser($params)
	{
		...
		$event = new Event("emercoinid", "OnAfterRegisterUserByNetwork", array($userId, $params['NETWORK_USER_ID'], $params['CLIENT_DOMAIN']));
		$event->send();
	...
	public static function initAdminPopup(array $params = array())
	{
		...
		\CJSCore::registerExt("emercoinid_admin", array(
			'js' => "/bitrix/js/emercoinid/ss_admin.js",
			'css' => "/bitrix/js/emercoinid/css/ss_admin.css",
		...
		\CJSCore::init(array("emercoinid_admin"));
	}
	...
	{
		return \CUserOptions::GetOption("emercoinid", "networkPopup", array("dontshow" => "N", "showcount" => 0));
	}
	...
	public static function setRegisterSettings($settings = array())
	{
		...
		{
			Option::set("emercoinid", "new_user_registration_network", $settings["REGISTER"] == "Y" ? "Y" : "N");
		}

		if(isset($settings["REGISTER_CONFIRM"]))
		{
			Option::set("emercoinid", "new_user_registration_confirm", $settings["REGISTER_CONFIRM"] == "Y" ? "Y" : "N");
		}

		if(isset($settings["REGISTER_WHITELIST"]))
		{
			$value = preg_split("/[^a-z0-9\-\.]+/", ToLower($settings["REGISTER_WHITELIST"]));
			Option::set("emercoinid", "new_user_registration_whitelist", serialize($value));
		}

		if(isset($settings["REGISTER_TEXT"]))
		{
			Option::set("emercoinid", "new_user_registration_text", trim($settings["REGISTER_TEXT"]));
		}

		if(isset($settings["REGISTER_SECRET"]))
		{
			Option::set("emercoinid", "new_user_registration_secret", trim($settings["REGISTER_SECRET"]));
		}
	...
	{
		return array(
			"REGISTER" => Option::get("emercoinid", "new_user_registration_network", "N"),
			"REGISTER_CONFIRM" => Option::get("emercoinid", "new_user_registration_confirm", "N"),
			"REGISTER_WHITELIST" => implode(';', unserialize(Option::get("emercoinid", "new_user_registration_whitelist", serialize(array())))),
			"REGISTER_TEXT" => Option::get("emercoinid", "new_user_registration_text", ""),
			"REGISTER_SECRET" => Option::get("emercoinid", "new_user_registration_secret", ""),
		);
	}
	...
	{
		return Option::get("emercoinid", "network_last_update_check", 0);
	}

	public static function setLastBroadcastCheck()
	{
		Option::set("emercoinid", "network_last_update_check", time());
	}
	...
	public static function processBroadcastData($data)
	{
		...
		foreach(GetModuleEvents("emercoinid", "OnNetworkBroadcast", true) as $eventHandler)
		{
