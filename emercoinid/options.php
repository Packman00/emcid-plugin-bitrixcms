...
$groupDenyAuth = CEmcAuth::getGroupsDenyAuth();
$groupDenySplit = CEmcAuth::getGroupsDenySplit();

$allowAuthorization = COption::GetOptionString("emercoinid", "allow_registration", "Y") == "Y";
...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["Update"].$_POST["Apply"].$_POST["RestoreDefaults"] <> '' && check_bitrix_sessid())
{
	...
	{
		COption::SetOptionString("emercoinid", "use_on_sites", serialize($_POST["use_on_sites"]));
		CAgent::RemoveModuleAgents("emercoinid");
		CAgent::AddAgent("CEmcAuthManager::SendSocialservicesMessages();", "emercoinid", "N", 100, "", "Y", "");
		CAgent::AddAgent("CEmcMessage::CleanUp();", "emercoinid", "N", 86400, "", "Y", "");
		foreach($arSiteList as $site)
		{
			$suffix = ($site <> ''? '_bx_site_'.$site:'');
			$siteId = ($site <> '' ? $site : SITE_ID);

			COption::SetOptionString("emercoinid", "auth_services".$suffix, serialize($_POST["AUTH_SERVICES".$suffix]));
	...
	if($_POST["allow_send_user_activity"] == 'Y')
		$allowSendActivity = 'Y';
	COption::SetOptionString("emercoinid", "allow_send_user_activity", $allowSendActivity);

	CEmcAuth::setGroupsDenyAuth($_REQUEST["group_deny_auth"]);
	CEmcAuth::setGroupsDenySplit($_REQUEST["group_deny_split"]);

	if(isset($_REQUEST["allow_registration"]))
	{
		COption::SetOptionString("emercoinid", "allow_registration", $_REQUEST["allow_registration"] == "N" ? "N" : "Y");
	}
		...
		<tr>
			<td> <?=GetMessage("soc_serv_send_activity");?> </td><td>
				<input type="checkbox" name="allow_send_user_activity" id="allow_send_user_activity" value="Y"
					<?if(COption::GetOptionString("emercoinid", "allow_send_user_activity", "N") == 'Y') echo " checked"; elseif(COption::GetOptionString("emercoinid", "allow_send_user_activity", false) === false)  echo " checked";?>>
...
?>
				<tr>
					...
					<td>&nbsp;</td>
					<td><a href="javascript:void(0)" onclick="MoveRowUp(this)"><img src="/bitrix/images/emercoinid/up.gif" width="16" height="16" alt="<?echo GetMessage("soc_serv_opt_up")?>" border="0"></a></td>
					<td><a href="javascript:void(0)" onclick="MoveRowDown(this)"><img src="/bitrix/images/emercoinid/down.gif" width="16" height="16" alt="<?echo GetMessage("soc_serv_opt_down")?>" border="0"></a></td>
				</tr>
<?
	endforeach;
?>
